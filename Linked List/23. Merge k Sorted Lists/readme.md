  ...
  **通常而言，在实际的k路归并算法题中，我们更推荐直接使用改进版本的胜者树——败者树进行快速实现。**
  ...
  ### 1.3. 败者树的数组表示和实现技巧
  这一部分实际上是整个本题最难的部分，如果没有一定的经验，面试中很难在短时间写出bugfree的败者树数组实现。
  
  虽然网上已经存在一些实现方法，例如https://www.cnblogs.com/iyjhabc/p/3141665.html ，但是在个人看来其中的实现方法并不直观，下面给出我的实现方案。该实现方案直接按照上面的败者树的图进行实现。
  
  下面先给出具体实现方法：
  
  + 败者树的数组实现方法：
  > 1. 确定数组的长度：假设为k路归并，则败者树的结点数目为2\*k，因此数组的长度为2\*k。
  > 2. 在数组中初始化叶子结点：
  > 3. 在数组中更新叶子结点：
  
  + 相关证明：
  > 1. 为什么k路归并的败者树的结点数目为2\*k？
  
    证明: 根据完全二叉树的性质，完全二叉树中度为1的结点最多只有1个或者0个，当度为1的结点的数目为1时，总结点数=叶子结点数*2，当度为0的结点数目为0时，总结点数=2*叶子结点数-1，
